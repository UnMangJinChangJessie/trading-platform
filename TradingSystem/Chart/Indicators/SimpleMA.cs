...
public class SimpleMA : Indicator {
  ...
  protected override void Append(object sender, Candle candle) {
    ...
    if (BaseChart.Count + 1 < Period) Average.Add(double.NaN);
    else if (BaseChart.Count + 1 == Period) Average.Add((double)BaseChart[..].Average(c => c.Close));
    else {
  ...
  protected override void Prepend(object sender, Candle candle) {
    if (BaseChart == null) return;
    Average.Insert(0, double.NaN);
    if (BaseChart.Count + 1 >= Period) Average[Period - 1] = (double)BaseChart[..Period].Average(c => c.Close);
  }
